...
|---|---|---|---|---|
| 1 | Headline scoreboard | transcript facts (revenue/EPS actual) + Alpha Vantage `EARNINGS_ESTIMATES` via `estimates.py` (analyst consensus, added 2026-09-22) | stat-tile row: value, yoy/qoq delta, and a beat/miss line vs. consensus where a real estimate exists | Each tile independently shows a yoy/qoq delta AND a beat-vs-consensus line -- the two answer different questions (how the business is trending vs. whether it surprised Wall Street) and neither substitutes for the other. No consensus field exists for gross margin or operating cash flow (confirmed against the real AV response shape), so those tiles' beat line honestly reads "no consensus estimate" rather than deriving one. No standalone prose -- the tiles are the whole section. |
...
| 6 | Forward guidance | transcript facts (new guidance) + Alpha Vantage `EARNINGS_ESTIMATES` via `estimates.py` (analyst consensus for the GUIDED period, not the reported one) | small table, "vs. consensus" column | **Corrected 2026-09-22** (was comparing guidance to the quarter that just happened -- that only shows deceleration/acceleration, not whether guidance is good or bad news). The real comparison is guided range vs. what analysts already expected for that SAME future period -- convert the guided growth range to dollars using the real prior-year comparable quarter (`sec_edgar.py`) and check whether the consensus estimate sits inside, above, or below it. AV's estimates cover only EPS and revenue -- margin/opex/tax-rate/FX guidance rows stay blank in the consensus column rather than guessed. |
